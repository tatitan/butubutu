...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body>
        ...
        <form action="/products" method="POST">
            ...
            <div class="title">
                <h2>Title</h2>
                <input type="text" name="product[title]" placeholder="タイトル" value="{{ old('product.title') }}"/>
                <p class="title_error" style="color:red">{{ $errors->first('product.title') }}</p>
            </div>
            <div class='description'>
                <h2>Description</h2>
                <textarea type="text" name="product[description]" placeholder="説明文" >{{ old('product.description') }}</textarea>
